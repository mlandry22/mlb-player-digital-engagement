...
LR_BASE = 0.01
LR_STOP = 2000  ### 5000 & 0.001; 500 & 0.01
LR_EVAL = 500
...
#     TREES = 1400 if target=='target4' else TREES
    ...
    lgb_model = lgb.train(params,
                    ...
                    early_stopping_rounds = 2000 if target=='target1' else 500,
    ...
    if not os.path.exists("./saved"):
        os.makedirs("./saved")
    lgb_model.save_model(f"./saved/lgb_{target}_v30_bfa.txt")
...
#     lgb_dval = lgb.Dataset(val[use_cols], val[target], free_raw_data=False)
    ...
    if not os.path.exists("./saved"):
        os.makedirs("./saved")
    lgb_model.save_model(f"./saved/lgb_{target}_v30_bfa_full.txt")
