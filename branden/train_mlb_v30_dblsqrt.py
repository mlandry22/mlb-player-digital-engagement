...
#train = pd.read_csv("./smallTrainUpdated.csv")
eng, games, rosters, p_box_scores, t_box_scores, transactions, standings, awards, events, p_twitter, t_twitter = get_unnested_data(train, train_colnames)

  
teams = pd.read_csv("./data/teams.csv")
players = pd.read_csv("./data/players.csv")
players['mlbDebutDate_yyyymmdd'] = pd.to_datetime(players["mlbDebutDate"]).dt.strftime("%Y%m%d").astype(float)
player_debuts = players[['playerId','mlbDebutDate_yyyymmdd']].set_index('playerId')['mlbDebutDate_yyyymmdd'].to_dict()
...
def check_debut(x, player_debuts):
    playerId = x['playerId']
    if not playerId in player_debuts:
        player_debuts[playerId] = np.nan
    if np.isnan(player_debuts[playerId]):
        if (x['gamesPlayedBatting']==1) | (x['gamesPlayedPitching']==1):
            player_debuts[playerId] = x['dailyDataDate']
            return 1
    else:
        if ((x['gamesPlayedBatting']==1) | (x['gamesPlayedPitching']==1)) & (x['dailyDataDate']==player_debuts[playerId]):
            return 1
        else:
            return 0
##################################################################################################
...
played_game = p_box_scores.groupby(['playerId','dailyDataDate'])['gamePk'].count().reset_index()
...
schedule = schedule[schedule['dailyDataDate']<20210601]
...
p_twitter_recent = pd.DataFrame()
p_twitter_delta = pd.DataFrame()
team_win_history = {}
win_streaks = {}
hitter_history_dict = {}
...
for i, data in tqdm(train.iterrows()):
    ...
    if not games.empty and not games.loc[games['gameType'].isin(["R", "F","D","L","W","C","P"])].empty:
        ...
        if not schedule_day.empty and not p_box_scores.empty:
            ...
            # p_box_scores['debut'] = p_box_scores.apply(lambda x: check_debut(x, player_debuts), axis=1)
...
#                 fielding_history_features[k][f'{feat}_last{day}'] = d_cumsum[day-1]
    ...
    t_tmp['ddd_month'] = np.floor(t_tmp.dailyDataDate/100).astype(int)
    ...
    ### END TRAILING AGGREGATION MERGES
...
tf = pd.concat(t)
tf['season'] = tf['season'].fillna(method='ffill').fillna(method='bfill')
#1216it [1:15:21,  3.72s/it]
...
# tf.to_csv("./tf_saved_725a.csv", index=False)
...
#     tf.shape
# except NameError:
#     #tf = pd.read_csv("./tf_saved_cleaned_TE.csv")
#     tf = pd.read_csv("./tf_saved_725a.csv")


# In[84]:


#te_monthly = pd.read_csv("./eng_moving_target_encodings.csv")
#te_monthly.head(1)
...
## remove from existing data frame
#tf = tf.drop(columns = ['target1_median', 'target2_median', 'target3_median', 'target4_median',
#                  'target1_var', 'target2_var','target3_var','target4_var',
#                   'target1_p_mean_game_day','target2_p_mean_game_day',
#                   'target3_p_mean_game_day','target4_p_mean_game_day',
#                   'target1_p_var_game_day','target2_p_var_game_day',
#                   'target3_p_var_game_day','target4_p_var_game_day'])


# In[86]:


#tf = tf.merge(te_monthly,how='left', on=['playerId','dailyDataDate'])


# In[87]:


#tf.to_csv("./tf_saved_cleaned_TEv2.csv", index=False)


# In[88]:


###   NOW RESTART KERNEL AND RECLAIM SPACE
...
#    tf.shape
#except NameError:
#    tf = pd.read_csv("./tf_saved_cleaned_TEv2.csv")
...
TRAIN_CUTOFF_DATE = 20210601
VAL_CUTOFF_DATE = 20210701
tr = tf[(tf['dailyDataDate']<TRAIN_CUTOFF_DATE)]
# val = tf[(tf['dailyDataDate']>=TRAIN_CUTOFF_DATE) & (tf['dailyDataDate']<VAL_CUTOFF_DATE)]
val = tf[(tf['dailyDataDate']>=TRAIN_CUTOFF_DATE)]


# In[92]:


## Cut to only active players
players = pd.read_csv('./data/players.csv')
active_players = players[players.playerForTestSetAndFuturePreds==True]
val = val[val.playerId.isin(active_players.playerId)]
...
# In[127]:
def rescaled_mae(preds, train_data):
    labels = (train_data.get_label()**2)**2
    preds = (preds**2)**2
    return 'mae', np.mean(np.abs(labels - preds)), False

LR_BASE = 0.01
LR_STOP = int(5/LR_BASE)  ### 5000 & 0.001; 500 & 0.01
LR_EVAL = int(LR_STOP/10)
...
for target in ['target1','target2','target3','target4']:
    ...
    params['learning_rate'] = LR_BASE #if target in ['target1', 'target4'] else 0.001
#     TREES = 17500 if target=='target1' else 8700
#     TREES = 2000 if target=='target3' else TREES
#     TREES = 1400 if target=='target4' else TREES
    
    lgb_dtrain = lgb.Dataset(tr[use_cols], np.sqrt(np.sqrt(tr[target])), free_raw_data=False)

    lgb_dval = lgb.Dataset(val[use_cols], np.sqrt(np.sqrt(val[target])), free_raw_data=False)
    ...
    lgb_model = lgb.train(params,
                    ...
                    num_boost_round = 200000, # 200000,
                    early_stopping_rounds = LR_STOP,
                    feval=rescaled_mae,
    ...
    if not os.path.exists("./saved"):
        os.makedirs("./saved")
    lgb_model.save_model(f"./saved/lgb_{target}_v30_dblsqrt_mayval.txt")
    ...
    print(model_dict[target]['imp'].sort_values('gain')[-40:])
    model_dict[target]['best_score'] = lgb_model.best_score

#     may_preds = lgb_model.predict(val_may[use_cols])**2**2
#     may_mae = np.mean(np.abs(val_may[target] - may_preds))
#     may_mae_list.append(may_mae)
#     print(f"May MAE: {may_mae}")

# print(may_mae_list)
# print(f"Overall May MAE: {np.mean(may_mae_list)}")
...
# for target in ['target1', 'target2', 'target3', 'target4']:
#     lgb_model = lgb.Booster(model_file=f"./saved/lgb_{target}_v30_mayval.txt")
#     model_dict[target] = {}
#     model_dict[target]['num_trees'] = lgb_model.num_trees()
# #     model_dict[target]['imp'] = pd.DataFrame({'features': lgb_model.feature_name(), 'gain': lgb_model.feature_importance()})
# #     print(model_dict[target]['imp'].sort_values('gain')[-50:])
# #     model_dict[target]['best_score'] = 1.11329
...
#     TREES = 1400 if target=='target4' else TREES
    ...
    lgb_dtrain = lgb.Dataset(tf[use_cols], np.sqrt(np.sqrt(tf[target])), free_raw_data=False)
...
#     lgb_dval = lgb.Dataset(val[use_cols], val[target], free_raw_data=False)
    ...
    if not os.path.exists("./saved"):
        os.makedirs("./saved")
    lgb_model.save_model(f"./saved/lgb_{target}_dblsqrt_full.txt")
