...
tf.columns = [x.replace(" ", "_") for x in tf.columns]
...
#                  'min_data_in_leaf': args.min_data_in_leaf,
              ...
              'grow_policy': 'lossguide', # only used if tree_method=='hist'
              'max_leaves': 73, # only used if grow_policy=='lossguide'
              'colsample_bytree': 0.7,
#                  'bagging_freq': {'type': int, 'default': 1},
              'subsample': 0.9,
#                  'bagging_seed': {'type': int, 'default': 321},
              ...
              'eval_metric': "mae",
#               'lambda_': 0.0,
              'alpha': 0.0, # only for Huber and Quantile regression
              'verbosity': 1,
              'nthread': 14,
              # 'seed': args.seed,
#              'top_rate': 0.8, # only for GOSS
#              'other_rate': 0.2, # only for GOSS
#                  'boost_from_average': {'type': lambda x: (str(x).lower() == 'true'), 'default': False},
#                  'reg_sqrt': {'type': lambda x: (str(x).lower() == 'true'), 'default': False}, # Change to false for regression_l2
...
excl_cols = ['hitterId','pitcherId','playerId','gamePk','dailyDataDate','engagementMetricsDate','target1','target2','target3','target4',
            ...
            'eng_monthday','ddd_monthday','DOB_monthday','birthday_ddd','birthday_eng']
...
def mae(predt: np.ndarray, dtrain: xgb.DMatrix) -> Tuple[str, float]:
    ...
    y = (dtrain.get_label()**2)**2
    predt = (predt**2)**2
    res = np.mean(np.abs(y-predt))
    return 'MAE', res

with open("./saved/xgb_v30_use_cols.txt", "w") as f:
...
for target in ['target1','target2','target3','target4'][3:]:
    ...
    if not os.path.exists("./saved"):
        os.makedirs("./saved")
    xgb_model.save_model(f"./saved/xgb_{target}_v30.txt")

    model_dict[target] = {}
    model_dict[target]['num_trees'] = xgb_model.best_ntree_limit
    model_dict[target]['imp'] = pd.DataFrame.from_dict(xgb_model.get_score(importance_type='gain'), orient='index')
    print(model_dict[target]['imp'].sort_values(0)[-50:])
...
#     xgb_model.load_model(f"./saved/xgb_{target}_v30.txt")
#     model_dict[target] = {}
#     model_dict[target]['num_trees'] = xgb_model.best_ntree_limit
#     model_dict[target]['imp'] = pd.DataFrame.from_dict(xgb_model.get_score(importance_type='gain'), orient='index')
#     model_dict[target]['best_score'] = xgb_model.best_score
...
#           verbose_eval=100
    ...
    if not os.path.exists("./saved"):
        os.makedirs("./saved")
    xgb_model.save_model(f"./saved/xgb_{target}_v30_full.txt")
